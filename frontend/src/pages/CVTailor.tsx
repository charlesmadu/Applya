    ...
    <div className="flex flex-col h-full space-y-6">
      ...
      <div>
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
          <Sparkles className="text-purple-600 dark:text-purple-400" /> AI CV Tailor
        </h2>
        <p className="text-slate-500 dark:text-slate-400">Customize your resume for specific job descriptions to increase your match score.</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6 flex flex-col">
            <div className="space-y-6 flex-1">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">1. Select Base Resume</label>
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setIsResumeDropdownOpen(!isResumeDropdownOpen)}
                    className="w-full flex items-center justify-between p-4 border border-slate-200 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-700 hover:border-purple-300 dark:hover:border-purple-600 transition-colors group"
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-600 text-purple-600 dark:text-purple-400 group-hover:border-purple-200 dark:group-hover:border-purple-600 transition-colors">
                        <FileText size={24} />
                      </div>
                      <div className="text-left">
                        <p className="font-medium text-slate-900 dark:text-white">{selectedResume.name}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">Updated {selectedResume.date} • {selectedResume.size}</p>
                      </div>
                    </div>
                  ...
                  {isResumeDropdownOpen && (
                    <div className="absolute z-10 w-full mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg overflow-hidden">
                      <div className="p-2 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
                        <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Your Resumes</p>
                      </div>
                        ...
                        {uploadedResumes.length === 0 ? (
                          ...
                          uploadedResumes.map(resume => (
                            <button
                              ...
                              className={`w-full flex items-center justify-between p-3 hover:bg-purple-50 dark:hover:bg-slate-700 transition-colors ${
                                selectedResume.id === resume.id ? 'bg-purple-50 dark:bg-slate-700' : ''
                              }`}
                            >
                              <div className="flex items-center gap-3">
                                <FileText size={18} className="text-slate-400 dark:text-slate-500" />
                                <div className="text-left">
                                  <p className="font-medium text-slate-800 dark:text-white text-sm">{resume.name}</p>
                                  <p className="text-xs text-slate-500 dark:text-slate-400">{resume.size} • {resume.date}</p>
                                </div>
                              </div>
                      ...
                      <div className="p-2 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
                        <button 
                          ...
                          className="text-xs font-medium text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 w-full text-left"
                        >
                          + Upload new resume
                        </button>
                      </div>
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">2. Job Description</label>
                  {jobDescription && (
                    <button 
                ...
                <textarea
                  ...
                  className="w-full h-64 flex-1 p-4 text-sm border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 outline-none transition-all"
                />
              ...
              <button 
                ...
                className={`w-full py-3 rounded-xl text-white font-medium flex items-center justify-center gap-2 transition-all ${
                  isGenerating || !jobDescription 
                    ...
                    : 'bg-purple-600 hover:bg-purple-700 shadow-lg hover:-translate-y-0.5'
                }`}
              >
                {isGenerating ? (
                  <>
          ...
          <Card className="p-0 overflow-hidden flex-1 flex flex-col">
            <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between">
              <h3 className="font-semibold text-slate-800 dark:text-white flex items-center gap-2">
                <Clock size={16} className="text-slate-400 dark:text-slate-500" /> Recent History
              </h3>
            ...
            <div className="flex-1 overflow-y-auto">
              {generatedCVs.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">No CVs generated yet</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Your tailored CVs will appear here</p>
                </div>
              ) : (
                  ...
                  {generatedCVs.map((item) => (
                    <div key={item.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                      <div className="flex justify-between items-start mb-1">
                        <h4 className="font-medium text-slate-800 dark:text-white text-sm group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors truncate pr-2">
                          {item.role}
                        </h4>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 mb-1 font-medium">{item.company}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 mb-3">Base: {item.baseResume}</p>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-1">
                          <button 
                            ...
                            className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-600 rounded-md transition-colors" 
                            title="Preview"
                          >
                            <Eye size={14} />
                          </button>
                          ...
                          {deleteConfirm === item.id ? (
                            <div className="flex items-center gap-1 bg-red-50 dark:bg-red-900/20 rounded px-1">
                              <button 
                                onClick={() => handleDeleteCV(item.id)}
                                className="text-red-600 dark:text-red-400 text-[10px] font-bold p-1"
                              >
                                Yes
                              </button>
                              <button 
                                onClick={() => setDeleteConfirm(null)}
                                className="text-slate-500 dark:text-slate-400 text-[10px] font-bold p-1"
                              >
                                No
                              </button>
                            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <Card className="w-full max-w-2xl p-6 mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-700 mb-4">
              <div>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white">{viewingCV.role}</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400">{viewingCV.company} • Generated {viewingCV.date}</p>
              </div>
              <button 
            ...
            <div className="space-y-4">
              <div>
                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Base Resume Used</h4>
                <div className="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                  <FileText size={16} className="text-purple-600 dark:text-purple-400" />
                  <span className="text-sm text-slate-700 dark:text-slate-200">{viewingCV.baseResume}</span>
                </div>
              </div>

              <div>
                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Job Description Preview</h4>
                <p className="text-sm text-slate-600 dark:text-slate-300 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
